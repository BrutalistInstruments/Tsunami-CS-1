...
#define MIDI_EVENT_BUFFER_SIZE 10 //we might want to make this bigger, since we will be recieving a lot of notes.
...
ISR (USART1_RX_vect) // interrupt service routine called each time USART data is recieved
{
  uint8_t data = UDR1;  // receives data from USART data register
...
void initMidi()
{
  ...
  UCSR1B = (1 << RXEN0) | (1 << TXEN0) | (1 << RXCIE0); // configuring USART (RXEN = reciever enable, TXEN = transmitter enable, RXCIE = reciever interrupt enable

  UCSR1C |=  0b00000110; ////(1 << USCZ11) | (1 << USCZ10); // for some reasone USCZ11 and USCZ10 are not recognized
}
...
void midiRead(uint8_t midiChannel, Pattern currentPattern)
{
  while (midiReadIndex != midiWriteIndex)
  {
    ...
    if ((midiMessageType==MIDI_NOTE_ON)&&(midiVelocity!=0)&&midiChannel==midiChannelIn)
    {
        for (int i=0; i<16; i++)
        {
          if(midiEventBuffer[midiReadIndex].dataByte[0]==currentPattern.midiTrackNote[i])
    ...
    /*
    midiTransmit(midiEventBuffer[midiReadIndex].statusByte);
    midiTransmit(midiEventBuffer[midiReadIndex].dataByte[0]);
    midiTransmit(midiEventBuffer[midiReadIndex].dataByte[1]);
    */
